...
class BooleanPayloadDecoder(IntegerPayloadDecoder):
    ...
    def _createComponent(self, asn1Spec, tagSet, value, **options):
        return IntegerPayloadDecoder._createComponent(self, asn1Spec, tagSet, value and 1 or 0, **options)
...
class BitStringPayloadDecoder(AbstractSimplePayloadDecoder):
    ...
    def valueDecoder(self, substrate, asn1Spec, tagSet=None, length=None, state=None, decodeFun=None, substrateFun=None, **options):
        ...
        if tagSet[0].tagFormat == tag.tagFormatSimple:  # XXX what tag to check?
            ...
            value = self.protoComponent.fromOctetString(chunk, internalFormat=True, padding=trailingBits)
        ...
        while substrate.tell() - current_position < length:
            ...
            bitString = self.protoComponent.fromOctetString(
                component[1:], internalFormat=True, prepend=bitString, padding=trailingBits
            )
...
class ConstructedPayloadDecoderBase(AbstractConstructedPayloadDecoder):
        ...
        if asn1Spec.typeId in (univ.Sequence.typeId, univ.Set.typeId):
            ...
            if LOG:
                LOG(
                    ...
                    % (not isDeterministic and "non-" or "", isSetType and "SET" or "", asn1Spec)
                )
    ...
    def indefLenValueDecoder(
        ...
        if asn1Spec is None:
            for asn1Object in self._decodeComponentsSchemaless(
                substrate, tagSet=tagSet, decodeFun=decodeFun, length=length, **dict(options, allowEoo=True)
            ):
                if isinstance(asn1Object, SubstrateUnderrunError):
                    yield asn1Object
...
class ChoicePayloadDecoder(ConstructedPayloadDecoderBase):
    ...
    def valueDecoder(self, substrate, asn1Spec, tagSet=None, length=None, state=None, decodeFun=None, substrateFun=None, **options):
        ...
        if asn1Object.tagSet == tagSet:
            ...
            for component in decodeFun(substrate, asn1Object.componentTagMap, **options):
                if isinstance(component, SubstrateUnderrunError):
                    yield component
            ...
            for component in decodeFun(substrate, asn1Object.componentTagMap, tagSet, length, state, **options):
                if isinstance(component, SubstrateUnderrunError):
                    yield component
        ...
        asn1Object.setComponentByType(
            effectiveTagSet, component, verifyConstraints=False, matchTags=False, matchConstraints=False, innerFlag=False
        )
    ...
    def indefLenValueDecoder(
        ...
        if LOG:
            LOG("decoding %s as %stagged CHOICE" % (tagSet, isTagged and "explicitly " or "un"))

        while True:

            if isTagged:
                iterator = decodeFun(substrate, asn1Object.componentType.tagMapUnique, **dict(options, allowEoo=True))

            else:
                iterator = decodeFun(
                    substrate, asn1Object.componentType.tagMapUnique, tagSet, length, state, **dict(options, allowEoo=True)
                )
...
class AnyPayloadDecoder(AbstractSimplePayloadDecoder):
    ...
    def valueDecoder(self, substrate, asn1Spec, tagSet=None, length=None, state=None, decodeFun=None, substrateFun=None, **options):
        ...
        if substrateFun:
            for chunk in substrateFun(self._createComponent(asn1Spec, tagSet, noValue, **options), substrate, length, options):
                yield chunk
...
class SingleItemDecoder(object):
    ...
    def __call__(
        self, substrate, asn1Spec=None, tagSet=None, length=None, state=stDecodeTag, decodeFun=None, substrateFun=None, **options
    ):
            ...
            if state is stDecodeLength:
                ...
                if firstOctet < 128:
                    ...
                    if len(encodedLength) != size:
                        raise error.SubstrateUnderrunError("%s<%s at %s" % (size, len(encodedLength), tagSet))
            ...
            if state is stGetValueDecoderByTag:
                ...
                if LOG:
                    LOG(
                        ...
                        % (
                            concreteDecoder and concreteDecoder.__class__.__name__ or "<none>",
            ...
            if state is stGetValueDecoderByAsn1Spec:
                ...
                if LOG:
                    LOG(
                        ...
                        % (
                            state is stDecodeValue and concreteDecoder.__class__.__name__ or "<none>",
                            state is stDecodeValue and "value" or "as explicit tag",
                        )
                    )
            ...
            if state is stDecodeValue:
                ...
                if length == -1:  # indef length
                    for value in concreteDecoder.indefLenValueDecoder(
                        substrate, asn1Spec, tagSet, length, stGetValueDecoder, self, substrateFun, **options
                    ):
                    ...
                    if bytesRead != length:
                        raise PyAsn1Error("Read %s bytes instead of expected %s." % (bytesRead, length))
