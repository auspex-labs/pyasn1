...
class AbstractItemEncoder(object):
    ...
    def encode(self, value, asn1Spec=None, encodeFun=None, **options):
        ...
        # untagged item?
        if not tagSet:
            substrate, isConstructed, isOctets = self.encodeValue(value, asn1Spec, encodeFun, **options)
            return substrate
        ...
        for idx, singleTag in enumerate(tagSet.superTags):
            ...
            if not idx:
                ...
                if LOG:
                    LOG("encoded %svalue %s into %s" % (isConstructed and "constructed " or "", value, substrate))
            ...
            if LOG:
                LOG(
                    ...
                    % (isConstructed and "constructed " or "", singleTag, debug.hexdump(ints2octs(header)))
                )

            header += self.encodeLength(len(substrate), defModeOverride)

            if LOG:
                LOG("encoded %s octets (tag + payload) into %s" % (len(substrate), debug.hexdump(ints2octs(header))))
...
class IntegerEncoder(AbstractItemEncoder):
    ...
    def encodeValue(self, value, asn1Spec, encodeFun, **options):
        if value == 0:
            if LOG:
                LOG("encoding %spayload for zero INTEGER" % (self.supportCompactZero and "no " or ""))
        ...
        if asn1Spec is None:
            ...
            for idx, component in enumerate(value.values()):
                ...
                if namedTypes and namedType.openType:
                    ...
                    if wrapType.typeId in (univ.SetOf.typeId, univ.SequenceOf.typeId):
                        ...
                        substrate += encodeFun(component, asn1Spec, **dict(options, wrapType=wrapType.componentType))
            ...
            for idx, namedType in enumerate(asn1Spec.componentType.namedTypes):
                ...
                if namedType.openType:
                    ...
                    if componentSpec.typeId in (univ.SetOf.typeId, univ.SequenceOf.typeId):

                        substrate += encodeFun(component, componentSpec, **dict(options, wrapType=componentSpec.componentType))
...
class ChoiceEncoder(AbstractItemEncoder):
    def encodeValue(self, value, asn1Spec, encodeFun, **options):
        if asn1Spec is None:
            ...
            names = [namedType.name for namedType in asn1Spec.componentType.namedTypes if namedType.name in value]
            if len(names) != 1:
...
class SingleItemEncoder(object):
    ...
    def __call__(self, value, asn1Spec=None, **options):
        try:
            ...
            raise error.PyAsn1Error("Value %r is not ASN.1 type instance " 'and "asn1Spec" not given' % (value,))
