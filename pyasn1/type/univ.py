        ...
        def __unicode__(self):
            try:
                ...
                raise error.PyAsn1UnicodeDecodeError(
                    "Can't decode string '%s' with codec " "%s" % (self._value, self.encoding), exc
                )
            ...
            if isinstance(value, bytes):
                ...
                try:
                    ...
                    raise error.PyAsn1UnicodeEncodeError(
                        "Can't encode string '%s' with '%s' " "codec" % (value, self.encoding), exc
                    )
...
class SequenceOf(SequenceOfAndSetOfBase):
    ...
    #: :py:class:`~pyasn1.type.tag.TagSet` object representing ASN.1 tag(s)
    #: associated with |ASN.1| type.
    tagSet = tag.initTagSet(tag.Tag(tag.tagClassUniversal, tag.tagFormatConstructed, 0x10))
...
class SetOf(SequenceOfAndSetOfBase):
    ...
    #: :py:class:`~pyasn1.type.tag.TagSet` object representing ASN.1 tag(s)
    #: associated with |ASN.1| type.
    tagSet = tag.initTagSet(tag.Tag(tag.tagClassUniversal, tag.tagFormatConstructed, 0x11))
...
class Set(SequenceAndSetBase):
    ...
    def getComponentByType(self, tagSet, default=noValue, instantiate=True, innerFlag=False):
        ...
        componentValue = self.getComponentByPosition(
            self.componentType.getPositionByType(tagSet), default=default, instantiate=instantiate
        )
