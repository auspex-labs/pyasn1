...
class TaggingTestSuite(TagSetTestCaseBase):
    def testImplicitTag(self):
        ...
        assert t == tag.TagSet(
            tag.Tag(tag.tagClassApplication, tag.tagFormatSimple, 12), tag.Tag(tag.tagClassApplication, tag.tagFormatSimple, 14)
...
class SuperTagSetTestCase(TagSetTestCaseBase):
    ...
    def testSuperTagCheck2(self):
        assert not self.ts1.isSuperTagSetOf(
            tag.TagSet(
                tag.Tag(tag.tagClassUniversal, tag.tagFormatSimple, 12), tag.Tag(tag.tagClassUniversal, tag.tagFormatSimple, 13)
            )
        ), "isSuperTagSetOf() fails"
