...
class IntegerTestCase(BaseTestCase):
    ...
    def testTag(self):
        assert univ.Integer().tagSet == tag.TagSet((), tag.Tag(tag.tagClassUniversal, tag.tagFormatSimple, 0x02))
...
class BooleanTestCase(BaseTestCase):
    ...
    def testTag(self):
        assert univ.Boolean().tagSet == tag.TagSet((), tag.Tag(tag.tagClassUniversal, tag.tagFormatSimple, 0x01))
...
class BitStringTestCase(BaseTestCase):
    ...
    def testTag(self):
        assert univ.BitString().tagSet == tag.TagSet((), tag.Tag(tag.tagClassUniversal, tag.tagFormatSimple, 0x03))
...
class OctetStringWithUnicodeMixIn(object):
    ...
    def testSerialised(self):
        if sys.version_info[0] < 3:
            assert (
                str(univ.OctetString(self.encodedPythonString, encoding=self.encoding)) == self.encodedPythonString
            ), "__str__() fails"
        else:
            assert (
                bytes(univ.OctetString(self.encodedPythonString, encoding=self.encoding)) == self.encodedPythonString
            ), "__str__() fails"
...
class OctetStringTestCase(BaseTestCase):
    ...
    def testTag(self):
        assert univ.OctetString().tagSet == tag.TagSet((), tag.Tag(tag.tagClassUniversal, tag.tagFormatSimple, 0x04))
...
class Null(BaseTestCase):
    ...
    def testTag(self):
        assert univ.Null().tagSet == tag.TagSet((), tag.Tag(tag.tagClassUniversal, tag.tagFormatSimple, 0x05))
...
class RealTestCase(BaseTestCase):
    ...
    def testTag(self):
        assert univ.Real().tagSet == tag.TagSet((), tag.Tag(tag.tagClassUniversal, tag.tagFormatSimple, 0x09))
...
class ObjectIdentifier(BaseTestCase):
    ...
    def testTag(self):
        assert univ.ObjectIdentifier().tagSet == tag.TagSet((), tag.Tag(tag.tagClassUniversal, tag.tagFormatSimple, 0x06))
...
class SequenceOf(BaseTestCase):
    ...
    def testTag(self):
        assert self.s1.tagSet == tag.TagSet((), tag.Tag(tag.tagClassUniversal, tag.tagFormatConstructed, 0x10)), "wrong tagSet"
    ...
    def testSubtypeSpec(self):
        s = self.s1.clone(componentType=univ.OctetString().subtype(subtypeSpec=constraint.SingleValueConstraint(str2octs("abc"))))
        try:
            s.setComponentByPosition(
                0, univ.OctetString().subtype("abc", subtypeSpec=constraint.SingleValueConstraint(str2octs("abc")))
            )
        except PyAsn1Error:
    ...
    def testConsistency(self):
        s = self.s1.clone(subtypeSpec=constraint.ConstraintsUnion(constraint.ValueSizeConstraint(1, 1)))
    ...
    def testGetComponentTagMap(self):
        assert self.s1.componentType.tagMap.presentTypes == {univ.OctetString.tagSet: univ.OctetString("")}

    def testSubtype(self):
        subtype = self.s1.subtype(
            implicitTag=tag.Tag(tag.tagClassPrivate, tag.tagFormatSimple, 2), subtypeSpec=constraint.ValueSizeConstraint(0, 1)
        )
        subtype.clear()
        clone = self.s1.clone(
            tagSet=tag.TagSet(tag.Tag(tag.tagClassPrivate, tag.tagFormatSimple, 2)),
    ...
    def testSetComponents(self):
        assert self.s1.clone().setComponents("abc", "def") == self.s1.setComponentByPosition(0, "abc").setComponentByPosition(
            1, "def"
        )
...
class Sequence(BaseTestCase):
    ...
    def testSetComponents(self):
        assert self.s1.clone().setComponents(name="a", nick="b", age=1) == self.s1.setComponentByPosition(
            0, "a"
        ).setComponentByPosition(1, "b").setComponentByPosition(2, 1)
    ...
    def testGetComponentWithConstructedDefault(self):
        class Sequence(univ.Sequence):
            componentType = namedtype.NamedTypes(
                namedtype.NamedType("name", univ.OctetString()),
                namedtype.DefaultedNamedType("nick", univ.SequenceOf(componentType=univ.Integer()).setComponentByPosition(0, 1)),
            )
    ...
    def testSchemaWithOptionalComponents(self):
        class Sequence(univ.Sequence):
            componentType = namedtype.NamedTypes(namedtype.OptionalNamedType("name", univ.OctetString()))
    ...
    def testSchemaWithOptionalComponents(self):
        class Sequence(univ.Sequence):
            componentType = namedtype.NamedTypes(namedtype.DefaultedNamedType("name", univ.OctetString("")))
    ...
    def testIsInconsistentWithComponentsConstraint(self):
        class Sequence(univ.Sequence):
            componentType = namedtype.NamedTypes(
                namedtype.OptionalNamedType("name", univ.OctetString()), namedtype.DefaultedNamedType("age", univ.Integer(65))
            )
            subtypeSpec = constraint.WithComponentsConstraint(
                ("name", constraint.ComponentPresentConstraint()), ("age", constraint.ComponentAbsentConstraint())
            )
...
class SequencePicklingTestCase(unittest.TestCase):
    def testSchemaPickling(self):
        old_asn1 = univ.Sequence(componentType=namedtype.NamedTypes(namedtype.NamedType("name", univ.OctetString())))
        serialised = pickle.dumps(old_asn1)
        assert serialised
        new_asn1 = pickle.loads(serialised)
...
class SetOf(BaseTestCase):
    ...
    def testTag(self):
        assert self.s1.tagSet == tag.TagSet((), tag.Tag(tag.tagClassUniversal, tag.tagFormatConstructed, 0x11)), "wrong tagSet"
...
class Set(BaseTestCase):
    ...
    def testByTypeWithPythonValue(self):
        ...
        assert self.s1.getComponentByType(univ.OctetString.tagSet) == str2octs("abc"), "set by name fails"
...
class SetPicklingTestCase(unittest.TestCase):
    def testSchemaPickling(self):
        old_asn1 = univ.Set(componentType=namedtype.NamedTypes(namedtype.NamedType("name", univ.OctetString())))
        serialised = pickle.dumps(old_asn1)
        assert serialised
        new_asn1 = pickle.loads(serialised)
...
class Choice(BaseTestCase):
    def setUp(self):
        ...
        innerComp = univ.Choice(
            componentType=namedtype.NamedTypes(
                namedtype.NamedType("count", univ.Integer()), namedtype.NamedType("flag", univ.Boolean())
            )
        )
        self.s1 = univ.Choice(
            componentType=namedtype.NamedTypes(
                namedtype.NamedType("name", univ.OctetString()), namedtype.NamedType("sex", innerComp)
            )
        )
    ...
    def testStaticDef(self):
        ...
        class OuterChoice(univ.Choice):
            componentType = namedtype.NamedTypes(
                namedtype.NamedType("name", univ.OctetString()), namedtype.NamedType("sex", InnerChoice())
            )
    ...
    def testGetComponentWithDefault(self):
        ...
        s = univ.Choice(
            componentType=namedtype.NamedTypes(
                namedtype.NamedType("name", univ.OctetString()), namedtype.NamedType("id", univ.Integer())
            )
        )
