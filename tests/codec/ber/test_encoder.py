...
class LargeTagEncoderTestCase(BaseTestCase):
    def setUp(self):
        ...
        self.o = univ.Integer().subtype(value=1, explicitTag=tag.Tag(tag.tagClassApplication, tag.tagFormatSimple, 0xDEADBEAF))
...
class IntegerEncoderTestCase(BaseTestCase):
    ...
    def testPosLong(self):
        assert encoder.encode(univ.Integer(0xFFFFFFFFFFFFFFFF)) == ints2octs((2, 9, 0, 255, 255, 255, 255, 255, 255, 255, 255))

    def testNegLong(self):
        assert encoder.encode(univ.Integer(-0xFFFFFFFFFFFFFFFF)) == ints2octs((2, 9, 255, 0, 0, 0, 0, 0, 0, 0, 1))
...
class BitStringEncoderTestCase(BaseTestCase):
    ...
    def testDefModeChunked(self):
        assert encoder.encode(self.b, maxChunkSize=1) == ints2octs((35, 8, 3, 2, 0, 169, 3, 2, 1, 138))

    def testIndefModeChunked(self):
        assert encoder.encode(self.b, defMode=False, maxChunkSize=1) == ints2octs((35, 128, 3, 2, 0, 169, 3, 2, 1, 138, 0, 0))
...
class BitStringEncoderWithSchemaTestCase(BaseTestCase):
    ...
    def testIndefModeChunked(self):
        assert encoder.encode(self.b, asn1Spec=self.s, defMode=False, maxChunkSize=1) == ints2octs(
...
class OctetStringEncoderTestCase(BaseTestCase):
    ...
    def testIndefModeChunked(self):
        assert encoder.encode(self.o, defMode=False, maxChunkSize=4) == ints2octs(
            (36, 128, 4, 4, 81, 117, 105, 99, 4, 4, 107, 32, 98, 114, 4, 4, 111, 119, 110, 32, 4, 3, 102, 111, 120, 0, 0)
        )
...
class OctetStringEncoderWithSchemaTestCase(BaseTestCase):
    ...
    def testIndefModeChunked(self):
        assert encoder.encode(self.o, asn1Spec=self.s, defMode=False, maxChunkSize=4) == ints2octs(
            (36, 128, 4, 4, 81, 117, 105, 99, 4, 4, 107, 32, 98, 114, 4, 4, 111, 119, 110, 32, 4, 3, 102, 111, 120, 0, 0)
        )
...
class ExpTaggedOctetStringEncoderTestCase(BaseTestCase):
    def setUp(self):
        ...
        self.o = univ.OctetString().subtype(
            value="Quick brown fox", explicitTag=tag.Tag(tag.tagClassApplication, tag.tagFormatSimple, 5)
        )
    ...
    def testIndefMode(self):
        assert encoder.encode(self.o, defMode=False) == ints2octs(
            (101, 128, 4, 15, 81, 117, 105, 99, 107, 32, 98, 114, 111, 119, 110, 32, 102, 111, 120, 0, 0)
        )

    def testDefModeChunked(self):
        assert encoder.encode(self.o, defMode=True, maxChunkSize=4) == ints2octs(
            (101, 25, 36, 23, 4, 4, 81, 117, 105, 99, 4, 4, 107, 32, 98, 114, 4, 4, 111, 119, 110, 32, 4, 3, 102, 111, 120)
        )
...
class ObjectIdentifierEncoderTestCase(BaseTestCase):
    def testOne(self):
        assert encoder.encode(univ.ObjectIdentifier((1, 3, 6, 0, 0xFFFFE))) == ints2octs((6, 6, 43, 6, 0, 191, 255, 126))
    ...
    def testEdge4(self):
        ...
        assert encoder.encode(univ.ObjectIdentifier((2, 0xFFFFFFFF))) == ints2octs((6, 5, 0x90, 0x80, 0x80, 0x80, 0x4F))
    ...
    def testEdge6(self):
        # 10000001|00000000
        assert encoder.encode(univ.ObjectIdentifier((2, 48))) == ints2octs((6, 2, 0x81, 0x00))

    def testEdge7(self):
        # 10000001|00110100|00000003
        assert encoder.encode(univ.ObjectIdentifier((2, 100, 3))) == ints2octs((6, 3, 0x81, 0x34, 0x03))

    def testEdge8(self):
        # 10000101|00000000
        assert encoder.encode(univ.ObjectIdentifier((2, 560))) == ints2octs((6, 2, 133, 0))

    def testEdge9(self):
        # 10001000|10000100|10000111|0000010
        assert encoder.encode(univ.ObjectIdentifier((2, 16843570))) == ints2octs((6, 4, 0x88, 0x84, 0x87, 0x02))
    ...
    def testLarge1(self):
        assert encoder.encode(univ.ObjectIdentifier((2, 18446744073709551535184467440737095))) == ints2octs(
            (0x06, 0x11, 0x83, 0xC6, 0xDF, 0xD4, 0xCC, 0xB3, 0xFF, 0xFF, 0xFE, 0xF0, 0xB8, 0xD6, 0xB8, 0xCB, 0xE2, 0xB7, 0x17)
        )

    def testLarge2(self):
        assert encoder.encode(univ.ObjectIdentifier((2, 999, 18446744073709551535184467440737095))) == ints2octs(
            (
                0x06,
                0x13,
                0x88,
...
class ObjectIdentifierWithSchemaEncoderTestCase(BaseTestCase):
    def testOne(self):
        assert encoder.encode((1, 3, 6, 0, 0xFFFFE), asn1Spec=univ.ObjectIdentifier()) == ints2octs((6, 6, 43, 6, 0, 191, 255, 126))


class RealEncoderTestCase(BaseTestCase):
    def testChar(self):
        assert encoder.encode(univ.Real((123, 10, 11))) == ints2octs((9, 7, 3, 49, 50, 51, 69, 49, 49))

    def testBin1(self):
        assert encoder.encode(univ.Real((0.5, 2, 0))) == ints2octs(  # default binEncBase = 2  # check encbase = 2 and exponent = -1
            (9, 3, 128, 255, 1)
        )
    ...
    def testBin3(self):
        ...
        assert encoder.encode(univ.Real((0.00390625, 2, 0))) == ints2octs((9, 3, 160, 254, 1))  # check encbase = 16
    ...
    def testBin4(self):
        ...
        assert encoder.encode(univ.Real((1, 2, 0))) == ints2octs((9, 3, 128, 0, 1))  # check exponent = 0
        encoder.TYPE_MAP[univ.Real.typeId].binEncBase = binEncBase
...
class UniversalStringEncoderWithSchemaTestCase(BaseTestCase):
    def testEncoding(self):
        assert encoder.encode(sys.version_info[0] >= 3 and "abc" or unicode("abc"), asn1Spec=char.UniversalString()) == ints2octs(
            (28, 12, 0, 0, 0, 97, 0, 0, 0, 98, 0, 0, 0, 99)
        ), "Incorrect encoding"
...
class BMPStringEncoderWithSchemaTestCase(BaseTestCase):
    def testEncoding(self):
        assert encoder.encode(sys.version_info[0] >= 3 and "abc" or unicode("abc"), asn1Spec=char.BMPString()) == ints2octs(
            (30, 6, 0, 97, 0, 98, 0, 99)
        ), "Incorrect encoding"
...
class UTF8StringEncoderWithSchemaTestCase(BaseTestCase):
    def testEncoding(self):
        assert encoder.encode(sys.version_info[0] >= 3 and "abc" or unicode("abc"), asn1Spec=char.UTF8String()) == ints2octs(
            (12, 3, 97, 98, 99)
        ), "Incorrect encoding"
...
class SequenceOfEncoderTestCase(BaseTestCase):
    ...
    def testDefModeChunked(self):
        ...
        assert encoder.encode(s, defMode=True, maxChunkSize=4) == ints2octs(
            (48, 19, 36, 17, 4, 4, 113, 117, 105, 99, 4, 4, 107, 32, 98, 114, 4, 3, 111, 119, 110)
        )
    ...
    def testIndefModeChunked(self):
        ...
        assert encoder.encode(s, defMode=False, maxChunkSize=4) == ints2octs(
            (48, 128, 36, 128, 4, 4, 113, 117, 105, 99, 4, 4, 107, 32, 98, 114, 4, 3, 111, 119, 110, 0, 0, 0, 0)
        )
...
class SequenceOfEncoderWithSchemaTestCase(BaseTestCase):
    ...
    def testDefModeChunked(self):
        assert encoder.encode(self.v, asn1Spec=self.s, defMode=True, maxChunkSize=4) == ints2octs(
            (48, 19, 36, 17, 4, 4, 113, 117, 105, 99, 4, 4, 107, 32, 98, 114, 4, 3, 111, 119, 110)
        )

    def testIndefModeChunked(self):
        assert encoder.encode(self.v, asn1Spec=self.s, defMode=False, maxChunkSize=4) == ints2octs(
            (48, 128, 36, 128, 4, 4, 113, 117, 105, 99, 4, 4, 107, 32, 98, 114, 4, 3, 111, 119, 110, 0, 0, 0, 0)
        )
...
class SetOfEncoderTestCase(BaseTestCase):
    ...
    def testDefModeChunked(self):
        ...
        s.setComponentByPosition(0, univ.OctetString("quick brown"))
        assert encoder.encode(s, defMode=True, maxChunkSize=4) == ints2octs(
            (49, 19, 36, 17, 4, 4, 113, 117, 105, 99, 4, 4, 107, 32, 98, 114, 4, 3, 111, 119, 110)
        )
...
class SequenceEncoderTestCase(BaseTestCase):
    ...
    def testIndefMode(self):
        assert encoder.encode(self.s, defMode=False) == ints2octs(
            (48, 128, 5, 0, 4, 11, 113, 117, 105, 99, 107, 32, 98, 114, 111, 119, 110, 2, 1, 1, 0, 0)
        )
    ...
    def testIndefModeChunked(self):
        assert encoder.encode(self.s, defMode=False, maxChunkSize=4) == ints2octs(
            (48, 128, 5, 0, 36, 128, 4, 4, 113, 117, 105, 99, 4, 4, 107, 32, 98, 114, 4, 3, 111, 119, 110, 0, 0, 2, 1, 1, 0, 0)
        )
...
class SequenceEncoderWithUntaggedOpenTypesTestCase(BaseTestCase):
    def setUp(self):
        ...
        self.s = univ.Sequence(
            componentType=namedtype.NamedTypes(
                namedtype.NamedType("id", univ.Integer()), namedtype.NamedType("blob", univ.Any(), openType=openType)
            )
        )
    ...
    def testEncodeOpenTypeChoiceOne(self):
        ...
        assert encoder.encode(self.s, asn1Spec=self.s) == ints2octs((48, 5, 2, 1, 1, 49, 50))
...
class SequenceEncoderWithImplicitlyTaggedOpenTypesTestCase(BaseTestCase):
    ...
    def testEncodeOpenTypeChoiceOne(self):
        ...
        assert encoder.encode(self.s, asn1Spec=self.s) == ints2octs((48, 9, 2, 1, 1, 131, 4, 131, 2, 49, 50))
...
class SequenceEncoderWithExplicitlyTaggedOpenTypesTestCase(BaseTestCase):
    ...
    def testEncodeOpenTypeChoiceOne(self):
        ...
        assert encoder.encode(self.s, asn1Spec=self.s) == ints2octs((48, 9, 2, 1, 1, 163, 4, 163, 2, 49, 50))
...
class SequenceEncoderWithUntaggedSetOfOpenTypesTestCase(BaseTestCase):
    def setUp(self):
        ...
        self.s = univ.Sequence(
            componentType=namedtype.NamedTypes(
                namedtype.NamedType("id", univ.Integer()),
                namedtype.NamedType("blob", univ.SetOf(componentType=univ.Any()), openType=openType),
            )
        )
    ...
    def testEncodeOpenTypeChoiceOne(self):
        ...
        assert encoder.encode(self.s, asn1Spec=self.s) == ints2octs((48, 7, 2, 1, 1, 49, 2, 49, 50))
    ...
    def testEncodeOpenTypeChoiceTwo(self):
        ...
        assert encoder.encode(self.s, asn1Spec=self.s) == ints2octs(
            (48, 16, 2, 1, 2, 49, 11, 113, 117, 105, 99, 107, 32, 98, 114, 111, 119, 110)
        )
...
class SequenceEncoderWithImplicitlyTaggedSetOfOpenTypesTestCase(BaseTestCase):
    def setUp(self):
        ...
        self.s = univ.Sequence(
            componentType=namedtype.NamedTypes(
                ...
                namedtype.NamedType(
                    "blob",
                    univ.SetOf(componentType=univ.Any().subtype(implicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatSimple, 3))),
                    openType=openType,
                ),
            )
        )
    ...
    def testEncodeOpenTypeChoiceOne(self):
        ...
        assert encoder.encode(self.s, asn1Spec=self.s) == ints2octs((48, 11, 2, 1, 1, 49, 6, 131, 4, 131, 2, 49, 50))
...
class SequenceEncoderWithExplicitlyTaggedSetOfOpenTypesTestCase(BaseTestCase):
    def setUp(self):
        ...
        self.s = univ.Sequence(
            componentType=namedtype.NamedTypes(
                ...
                namedtype.NamedType(
                    "blob",
                    univ.SetOf(componentType=univ.Any().subtype(explicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatSimple, 3))),
                    openType=openType,
                ),
            )
        )
    ...
    def testEncodeOpenTypeChoiceOne(self):
        ...
        assert encoder.encode(self.s, asn1Spec=self.s) == ints2octs((48, 11, 2, 1, 1, 49, 6, 163, 4, 163, 2, 49, 50))
...
class SequenceEncoderWithComponentsSchemaTestCase(BaseTestCase):
    ...
    def testWithOptionalIndefMode(self):
        self.__initWithOptional()
        assert encoder.encode(self.s, defMode=False) == ints2octs(
            (48, 128, 5, 0, 4, 11, 113, 117, 105, 99, 107, 32, 98, 114, 111, 119, 110, 0, 0)
        )

    def testWithOptionalDefModeChunked(self):
        self.__initWithOptional()
        assert encoder.encode(self.s, defMode=True, maxChunkSize=4) == ints2octs(
            (48, 21, 5, 0, 36, 17, 4, 4, 113, 117, 105, 99, 4, 4, 107, 32, 98, 114, 4, 3, 111, 119, 110)
        )
...
class ExpTaggedSequenceComponentEncoderTestCase(BaseTestCase):
    def setUp(self):
        ...
        self.s = univ.Sequence(
            componentType=namedtype.NamedTypes(
                namedtype.NamedType(
                    "number", univ.Boolean().subtype(explicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatSimple, 0))
                ),
            )
        )
...
class SetEncoderTestCase(BaseTestCase):
    ...
    def testIndefMode(self):
        assert encoder.encode(self.s, defMode=False) == ints2octs(
            (49, 128, 5, 0, 4, 11, 113, 117, 105, 99, 107, 32, 98, 114, 111, 119, 110, 2, 1, 1, 0, 0)
        )
    ...
    def testIndefModeChunked(self):
        assert encoder.encode(self.s, defMode=False, maxChunkSize=4) == ints2octs(
            (49, 128, 5, 0, 36, 128, 4, 4, 113, 117, 105, 99, 4, 4, 107, 32, 98, 114, 4, 3, 111, 119, 110, 0, 0, 2, 1, 1, 0, 0)
        )
...
class SetEncoderWithComponentsSchemaTestCase(BaseTestCase):
    ...
    def testDefModeChunked(self):
        self.__init()
        assert encoder.encode(self.s, defMode=True, maxChunkSize=4) == ints2octs((49, 2, 5, 0))

    def testIndefModeChunked(self):
        self.__init()
        assert encoder.encode(self.s, defMode=False, maxChunkSize=4) == ints2octs((49, 128, 5, 0, 0, 0))
    ...
    def testWithOptionalIndefMode(self):
        self.__initWithOptional()
        assert encoder.encode(self.s, defMode=False) == ints2octs(
            (49, 128, 5, 0, 4, 11, 113, 117, 105, 99, 107, 32, 98, 114, 111, 119, 110, 0, 0)
        )

    def testWithOptionalDefModeChunked(self):
        self.__initWithOptional()
        assert encoder.encode(self.s, defMode=True, maxChunkSize=4) == ints2octs(
            (49, 21, 5, 0, 36, 17, 4, 4, 113, 117, 105, 99, 4, 4, 107, 32, 98, 114, 4, 3, 111, 119, 110)
        )
    ...
    def testWithDefaultedIndefMode(self):
        self.__initWithDefaulted()
        assert encoder.encode(self.s, defMode=False) == ints2octs((49, 128, 5, 0, 2, 1, 1, 0, 0))

    def testWithDefaultedDefModeChunked(self):
        self.__initWithDefaulted()
        assert encoder.encode(self.s, defMode=True, maxChunkSize=4) == ints2octs((49, 5, 5, 0, 2, 1, 1))

    def testWithDefaultedIndefModeChunked(self):
        self.__initWithDefaulted()
        assert encoder.encode(self.s, defMode=False, maxChunkSize=4) == ints2octs((49, 128, 5, 0, 2, 1, 1, 0, 0))
...
class ChoiceEncoderTestCase(BaseTestCase):
    ...
    def testIndefMode(self):
        s = univ.Choice()
        s.setComponentByPosition(0, univ.OctetString("quick brown"))
        assert encoder.encode(s, defMode=False) == ints2octs((4, 11, 113, 117, 105, 99, 107, 32, 98, 114, 111, 119, 110))

    def testDefModeChunked(self):
        s = univ.Choice()
        s.setComponentByPosition(0, univ.OctetString("quick brown"))
        assert encoder.encode(s, defMode=True, maxChunkSize=4) == ints2octs(
            (36, 17, 4, 4, 113, 117, 105, 99, 4, 4, 107, 32, 98, 114, 4, 3, 111, 119, 110)
        )
    ...
    def testIndefModeChunked(self):
        ...
        s.setComponentByPosition(0, univ.OctetString("quick brown"))
        assert encoder.encode(s, defMode=False, maxChunkSize=4) == ints2octs(
            (36, 128, 4, 4, 113, 117, 105, 99, 4, 4, 107, 32, 98, 114, 4, 3, 111, 119, 110, 0, 0)
        )
