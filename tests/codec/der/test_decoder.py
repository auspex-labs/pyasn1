...
class BitStringDecoderTestCase(BaseTestCase):
    ...
    def testIndefMode(self):
        try:
            decoder.decode(ints2octs((35, 128, 3, 2, 0, 169, 3, 2, 1, 138, 0, 0)))
        except PyAsn1Error:
            pass
        else:
...
class OctetStringDecoderTestCase(BaseTestCase):
    def testShortMode(self):
        assert decoder.decode("\004\017Quick brown fox".encode()) == ("Quick brown fox".encode(), "".encode())

    def testIndefMode(self):
        try:
            decoder.decode(ints2octs((36, 128, 4, 15, 81, 117, 105, 99, 107, 32, 98, 114, 111, 119, 110, 32, 102, 111, 120, 0, 0)))
        except PyAsn1Error:
            pass
        else:
            assert 0, "indefinite length encoding tolerated"
...
class SequenceDecoderWithImplicitlyTaggedOpenTypesTestCase(BaseTestCase):
    ...
    def testDecodeOpenTypesChoiceOne(self):
        s, r = decoder.decode(ints2octs((48, 8, 2, 1, 1, 131, 3, 2, 1, 12)), asn1Spec=self.s, decodeOpenTypes=True)
        assert not r
        assert s[0] == 1
        assert s[1] == 12

    def testDecodeOpenTypesUnknownId(self):
        s, r = decoder.decode(ints2octs((48, 8, 2, 1, 3, 131, 3, 2, 1, 12)), asn1Spec=self.s, decodeOpenTypes=True)
        assert not r
        assert s[0] == 3
...
class SequenceDecoderWithExplicitlyTaggedOpenTypesTestCase(BaseTestCase):
    ...
    def testDecodeOpenTypesChoiceOne(self):
        s, r = decoder.decode(ints2octs((48, 8, 2, 1, 1, 163, 3, 2, 1, 12)), asn1Spec=self.s, decodeOpenTypes=True)
        assert not r
        assert s[0] == 1
        assert s[1] == 12
...
class SequenceDecoderWithUnaggedSetOfOpenTypesTestCase(BaseTestCase):
    ...
    def testDecodeOpenTypesChoiceOne(self):
        s, r = decoder.decode(ints2octs((48, 8, 2, 1, 1, 49, 3, 2, 1, 12)), asn1Spec=self.s, decodeOpenTypes=True)
        assert not r
        assert s[0] == 1
        assert s[1][0] == 12
    ...
    def testDecodeOpenTypesChoiceTwo(self):
        s, r = decoder.decode(
            ints2octs((48, 18, 2, 1, 2, 49, 13, 4, 11, 113, 117, 105, 99, 107, 32, 98, 114, 111, 119, 110)),
            asn1Spec=self.s,
            decodeOpenTypes=True,
        )
        assert not r
    ...
    def testDecodeOpenTypesUnknownId(self):
        s, r = decoder.decode(ints2octs((48, 8, 2, 1, 3, 49, 3, 2, 1, 12)), asn1Spec=self.s, decodeOpenTypes=True)
        assert not r
        assert s[0] == 3
        assert s[1][0] == univ.OctetString(hexValue="02010c")

    def testDontDecodeOpenTypesChoiceOne(self):
        s, r = decoder.decode(ints2octs((48, 8, 2, 1, 1, 49, 3, 2, 1, 12)), asn1Spec=self.s)
        assert not r
        assert s[0] == 1
    ...
    def testDontDecodeOpenTypesChoiceTwo(self):
        s, r = decoder.decode(
            ints2octs((48, 18, 2, 1, 2, 49, 13, 4, 11, 113, 117, 105, 99, 107, 32, 98, 114, 111, 119, 110)), asn1Spec=self.s
        )
        assert not r
        assert s[0] == 2
